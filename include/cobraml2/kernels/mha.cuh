...
// kernel 1 (QK^T):
__global__ void qk_kernel(float *Q, float *K, float *S, int B, int H, int N,
                          int d) {
...
// kernel 3 (PV):
__global__ void pv_kernel(float *P, float *V, float *O, int B, int H, int N,
                          int d) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  int total = B * H * N * d;
...
// launcher
void mha_forward(float *Q, float *K, float *V, float *O, int B, int H, int N,
                 int d) {
  ...
  // Launch kernels
  int threads = 256;
  qk_kernel<<<(B * H * N * N + threads - 1) / threads, threads>>>(Q, K, S, B, H,
                                                                  N, d);
  softmax_kernel<<<(B * H * N + threads - 1) / threads, threads>>>(S, P, B, H,
                                                                   N);
  pv_kernel<<<(B * H * N * d + threads - 1) / threads, threads>>>(P, V, O, B, H,
                                                                  N, d);
