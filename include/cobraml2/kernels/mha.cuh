...
template <int kBlockM = 64, int kBlockN = 64, int kBlockK = 32, int kHeadDim = 64>
struct MHAConfig {
  ...
  static constexpr int BlockK = kBlockK;      // tile size for head dimension (for QK^T)
...
__global__ void qk_kernel_cute(
    float const* Q_ptr,  // [B, N, H, d] - BSHD layout
  ...
  Tensor gQ = make_tensor(make_gmem_ptr(Q_ptr + qk_offset), shape_QK, stride_QK); // (n, d)
  Tensor gK = make_tensor(make_gmem_ptr(K_ptr + qk_offset), shape_QK, stride_QK); // (n, d)
  
  // s for this (b,h): offset = b*(H*N*N) + h*(N*N)
  int s_offset = b * (H * N * N) + h * (N * N);
  Tensor gS = make_tensor(make_gmem_ptr(S_ptr + s_offset), shape_S, stride_S); // (n, n)
  
  // define block tiler
  auto blk_shape = make_shape(Int<Config::BlockM>{}, Int<Config::BlockN>{}, Int<Config::BlockK>{});
  ...
  Tensor gQ_blk = local_tile(gQ, select<0,2>(blk_shape), blk_coord_Q); // (block_m, block_k, k_tiles)
  Tensor gK_blk = local_tile(gK, select<1,2>(blk_shape), blk_coord_K); // (block_n, block_k, k_tiles)
  Tensor gS_blk = local_tile(gS, select<0,1>(blk_shape), blk_coord_S); // (block_m, block_n)
  ...
  auto sQ_layout = make_layout(make_shape(Int<Config::BlockM>{}, Int<Config::BlockK>{}));
  auto sK_layout = make_layout(make_shape(Int<Config::BlockN>{}, Int<Config::BlockK>{}));
  ...
  Tensor tQgQ = local_partition(gQ_blk, thr_layout_copy, threadIdx.x); // (thr_m, thr_k, k_tiles)
  Tensor tQsQ = local_partition(sQ, thr_layout_copy, threadIdx.x); // (thr_m, thr_k)
  Tensor tKgK = local_partition(gK_blk, thr_layout_copy, threadIdx.x); // (thr_n, thr_k, k_tiles)
  Tensor tKsK = local_partition(sK, thr_layout_copy, threadIdx.x); // (thr_n, thr_k)
  
  // partition for MMA (shared memory and output)
  Tensor tCsQ = local_partition(sQ, thr_layout_mma, threadIdx.x, Step<_1, X>{}); // (thr_m, block_k)
  Tensor tCsK = local_partition(sK, thr_layout_mma, threadIdx.x, Step< X,_1>{}); // (thr_n, block_k)
  Tensor tCgS = local_partition(gS_blk, thr_layout_mma, threadIdx.x); // (thr_m, thr_n)
...
// softmax + pv kernel
template <class Config>
__global__ void softmax_pv_kernel_cute(
    float const* S_ptr, // [B, H, N, N] - Attention scores
...
void mha_forward(float* Q, float* K, float* V, float* O, int B, int N, int H, int d) {
  ...
  // Kernel 1: QK^T
  dim3 grid_qk(B * H, 
               (N + Config::BlockM - 1) / Config::BlockM,
  ...
  softmax_pv_kernel_cute<Config><<<blocks_softmax, threads_softmax>>>(S, V, O, B, N, H, d);
