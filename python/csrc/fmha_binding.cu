...
template <int H_start, int H_stop, int H_step, int D_start, int D_stop, int D_step>                                                                                                                                                                                                              
struct FMHADispatcher {
    ...
    using KernelFn = void(*)(float*, float*, float*, float*, uint32_t, uint32_t);
    ...
    void init_impl(std::index_sequence<Is...>){
        (..., register_one<Is>());
    }

    FMHADispatcher(){
        init_impl(std::make_index_sequence<configs.size()>{});
    }
...
torch::Tensor fmha_forward(                                                                                         
    ...
    TORCH_CHECK(
        Q_dtype == K_dtype && Q_dtype == V_dtype, 
