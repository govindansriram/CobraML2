...
void fill_random_uniform_impl(DType * data, int length, int seed, auto assignment_func){
    curandGenerator_t gen;
    thrust::device_vector<float> temp_dev_vec_float(length);

    curandCreateGenerator(&gen, CURAND_RNG_PSEUDO_DEFAULT);
    curandSetPseudoRandomGeneratorSeed(gen, seed);
    curandGenerateUniform(gen, thrust::raw_pointer_cast(temp_dev_vec_float.data()), length);
    ...
    cudaMemcpy(
        data, 
        thrust::raw_pointer_cast(temp_host_vec_dtype.data()),
        sizeof(DType) * length,
...
template<typename DType>
auto seeded_fill_random_uniform(int seed) {
    return [=](DType * data, int length){
        fill_random_uniform(data, length, seed);
    };
}

template<typename DType>
thrust::device_vector<DType> create_tensor(
    size_t length,
    auto fill_fn
){
