...
__global__ void prefix_scan_kernel(const int *in, int *out, int *block_prefix,
                                   int n) {
...
template <size_t warps_per_block>
__global__ void prefix_add(int *out, int *block_prefix, int n) {
...
void prefix_scan(const int *in, int *out, int n) {
  ...
  prefix_scan_kernel<num_warps>
      <<<num_blocks, block_dim>>>(in, out, block_prefixes_in, n);
  prefix_scan_kernel<num_warps><<<1, block_dim>>>(
      block_prefixes_in, block_prefixes_out, nullptr, num_blocks);
...
TEST(WarpScan, AllOnes) {
  ...
  prefix_scan(thrust::raw_pointer_cast(d_in.data()),
              thrust::raw_pointer_cast(d_out.data()), 32);
...
TEST(WarpScan, MediumSize1000) {
  ...
  for (int i = 0; i < N; i++) {
    ASSERT_EQ(out[i], expected[i])
        << "Mismatch at index " << i << " (input[i]=" << in[i] << ")";
  }
}
...
TEST(WarpScan, LargeSize100k) {
  ...
  thrust::host_vector<int> in(N);
  for (int i = 0; i < N; i++)
    in[i] = (i % 7) - 3; // small values to avoid overflow
