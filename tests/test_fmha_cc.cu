...
void test_fmha(int batch_size, int sequence_length) {
  using MHAType = kernels::FMHA<head_count, head_dim, B_r, B_c, float, 128, mask>;
...
TEST(FMHA_CC, H16_D64_Br64_Bc64_B4_N512_causal) { test_fmha<16, 64, 64, 64, true>(4, 512); }
...
// uneven block size by sequence length (requires predication) with causal masking
TEST(FMHA_CC, H2_D64_Br64_Bc64_B56_N490_causal) { test_fmha<2, 64, 64, 64, true>(56, 490); }
...
TEST(FMHA_CC, H16_D64_Br64_Bc64_B8_N64_causal) { test_fmha<16, 64, 64, 64, true>(8, 64); }
...
TEST(FMHA_CC, H16_D64_Br64_Bc64_B8_N59_causal) { test_fmha<16, 64, 64, 64, true>(8, 59); }